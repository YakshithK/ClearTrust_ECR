...
def predict(text, top_n=5):
    ...
    if int(scam_probability*100) > 50:
        send_sms_alert('Your family member just got a message that was most likely a scam ({0}%) : {1}. Here are the keywords: {2}.'.format(int(scam_probability), text, ", ".join(scam_keywords)))
